...
export default function Recommend() {
  ...
  if (!status || !info) return <LoaderComponent />;
  if (status !== "authenticated") return <Signup />;

  return info ? (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-black via-[#181818] to-black">
      <div className="flex justify-between p-4">
        <div className="italic text-2xl md:text-6xl mt-2 md:mt-1 font-bold text-[#e50914] underline decoration-[#e50914] drop-shadow-lg">
          Recommend'o
        </div>
          ...
          <div className="bg-[#181818] rounded-lg shadow-xl p-4 sm:p-6 border border-[#e50914]/30">
            <div className="grid grid-cols-2 gap-4 sm:gap-6 items-start">
              ...
              <div className="flex flex-col justify-start space-y-2 sm:space-y-3 text-white text-xs sm:text-sm md:text-base">
                ...
                <p className="italic">
                  <strong className="text-[#e50914]">Plot:</strong>{" "}
                  <span className="text-gray-300">
                    {isLargeScreen ? info?.Plot : info?.Plot.slice(0, 150) + "..."}
                  </span>
                </p>
      ...
      <PostReview film={movieName} dp={dp || '/EmptyImage.png'} />

      <div className="flex flex-col md:flex-row mt-8 md:px-4 gap-6">
        <div className="w-full lg:w-1/2 px-4">
          {reviews && reviews.length > 0 ? (
            <>
              <div className="font-bold text-2xl italic text-white mb-6 ml-2">Popular Reviews</div>
              {reviews.slice(l, l + 2).map((rev, index) => (
                <ReviewCard
                  key={index}
                  dp={users[l + index]?.dp || "/EmptyImage.png"}
                  review={rev.review}
            ...
            {chats.map((c) => (
              <div key={c.id} className={`flex ${c.user === "You" ? "justify-end" : "justify-start"}`}>
                <div className="rounded-full h-8 w-8 mr-2 mt-1 overflow-hidden">
                  <img src={c.dp || dp || '/EmptyImage.png'} alt="profilephoto" className="h-full w-full object-cover" />
                </div>
